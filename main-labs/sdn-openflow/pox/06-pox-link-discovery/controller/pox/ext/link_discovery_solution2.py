		...
		for sid in self.switch_id:
			...
			for port in self.switches[dpid]: # iterate over all the ports of the current switch
				if port.port_no != of.OFPP_CONTROLLER: # if the current port is not the one connecting to the controller
					...
					mac_dst = EthAddr("00:00:00:00:"+str(sid)+":"+str(port.port_no)) # encode in the mac_dst the relevant information
					...
					msg.data = ether.pack()
					msg.actions.append(of.ofp_action_output(port = port.port_no)) # instruct the switch to send the packet out of the current inspected port
